  ...
  return (
    <ul className="space-y-2">
      {filtered.map((s) => (
        <li
          key={s.id}
          className="flex items-center justify-between rounded-xl px-4 py-3 bg-surface border border-border hover:bg-secondary/20"
        >
          <div>
            <span className="font-medium text-primary">{s.title}</span>
            <span className="text-sm text-muted ml-2">— {s.candidate.name}</span>
            <span className="text-xs text-muted ml-2">({s.status})</span>
          </div>
