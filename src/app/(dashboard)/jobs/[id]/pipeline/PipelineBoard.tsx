  ...
  return (
    <div className="flex gap-4 overflow-x-auto pb-4 min-h-[400px]">
      {job.stages.map((stage) => (
        <div
          key={stage.id}
          className="w-72 shrink-0 rounded-2xl border border-chrome-border bg-surface shadow-sm"
        >
          <div className="px-4 py-3 border-b border-chrome-border bg-chrome/20">
            ...
            <p className="text-xs text-muted">{stage.candidates.length} applicants</p>
          </div>
          <CardContent className="p-2 space-y-2">
            {stage.candidates.map((c) => (
              <Card key={c.id} className="border-chrome-border p-3 hover:shadow-sm transition-subtle">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-muted truncate">{c.id.slice(0, 8)}</p>
                    </div>
                  </div>
                ...
                <p className="text-[10px] text-muted mt-1">1 min ago</p>
