...
export async function seedDemo(prisma: PrismaClient) {
  ...
  await prisma.user.upsert({
    where: { email: "superadmin@zecode.io" },
    create: {
      email: "superadmin@zecode.io",
      passwordHash: hash,
      name: "Aanya Sharma",
      role: "SUPER_ADMIN",
      organizationId: org.id,
    },
    update: { name: "Aanya Sharma", role: "SUPER_ADMIN" },
  });

  await prisma.user.upsert({
    where: { email: "admin@zecode.io" },
    create: {
      email: "admin@zecode.io",
      passwordHash: hash,
      name: "Admin",
      role: "ADMIN",
      organizationId: org.id,
  ...
  await prisma.user.upsert({
    where: { email: "manager@zecode.io" },
    create: {
      email: "manager@zecode.io",
      passwordHash: hash,
      name: "Hiring Manager",
      role: "HIRING_MANAGER",
      organizationId: org.id,
  ...
  const adminUser = await prisma.user.findUnique({ where: { email: "admin@zecode.io" } });
  ...
  for (let i = 0; i < 2; i++) {
    ...
    await prisma.interviewSummary.upsert({
      ...
      create: {
        ...
        mappedTraits: JSON.stringify({
          "Leadership Drive": "Led team under pressure",
          Communication: "Clear standups and stakeholder dialogue",
        }),
  ...
  await prisma.interviewSummary.upsert({
    ...
    create: {
      ...
      mappedTraits: JSON.stringify({
        ...
        Accountability: "Notice period stated",
      }),
      risks: JSON.stringify([]),
    },
    update: {},
    ...
    if (!existingSummaryTrans) {
      await prisma.translationRecord.create({
        data: {
          ...
          toLang: "de",
          translatedText:
            "Der Kandidat zeigte starke Führungsqualitäten und Kommunikation. Konkretes Beispiel für Führung unter Druck mit klaren Ergebnissen.",
  ...
  const cleanHumanText =
    "I chose to work on the payment flow because our support tickets showed a 30% drop-off at step 2. I talked to three customers and read the existing code. I changed the validation order and added a clearer error message. It took me about four hours. I'm not sure if we should add a progress bar—I'd want to A/B test that.";
  const aiAssistedText =
    "In conclusion, the payment flow optimization was undertaken following a comprehensive analysis of user behavior metrics. Key initiatives included refactoring the validation logic and implementing user-centric error messaging. Moving forward, we recommend conducting further research to validate the proposed progress indicator implementation.";
